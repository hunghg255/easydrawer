...
import {
  type ComponentAddedListener,
  type ImageLoader,
  type OnDetermineExportRectListener,
  type OnProgressListener,
} from '../types';
...
export default class SVGLoader implements ImageLoader {
  ...
  private getStyle(node: SVGElement, computedStyles?: CSSStyleDeclaration) {
    ...
    if (strokeAttribute && strokeWidthAttr) {
      try {
        let width = Number.parseFloat(strokeWidthAttr ?? '1');
        if (!isFinite(width)) {
          width = 0;
        }
    ...
    if (node.classList.contains(backgroundTypeToClassNameMap[BackgroundType.Grid])) {
      ...
      for (const className of node.classList) {
        if (className.startsWith(imageBackgroundGridSizeCSSPrefix)) {
          ...
          gridSize = Number.parseFloat(sizeStr.replace(/p/g, '.'));
        }
      }

      let gridStrokeWidth: number | undefined = undefined;
      if (gridStrokeWidthStr) {
        gridStrokeWidth = Number.parseFloat(gridStrokeWidthStr);
      }
  ...
  private getTransform(
    ...
    if (!transform) {
      ...
      if (elemX || elemY) {
        const x = Number.parseFloat(elemX ?? '0');
        const y = Number.parseFloat(elemY ?? '0');
        if (!isNaN(x) && !isNaN(y)) {
  ...
  private makeText(elem: SVGTextElement | SVGTSpanElement): TextComponent {
    ...
    if (elemDX) {
      ...
      transform = transform.rightMul(Mat33.translation(Vec2.of(Number.parseFloat(elemDX), 0)));
    ...
    if (elemDY) {
      ...
      transform = transform.rightMul(Mat33.translation(Vec2.of(0, Number.parseFloat(elemDY))));
  ...
  private updateViewBox(node: SVGSVGElement) {
    ...
    const y = Number.parseFloat(components[1]);
    const width = Number.parseFloat(components[2]);
    const height = Number.parseFloat(components[3]);
