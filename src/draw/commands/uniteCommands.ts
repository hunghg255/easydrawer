 ...
 * ```
 */
function uniteCommands <T extends Command>(commands: T[],
  options?: UniteCommandsOptions | number): T extends SerializableCommand ? SerializableCommand : Command {
...
SerializableCommand.register('union', (data: any, editor) => {
  if (typeof data.data.length !== 'number') {
    throw new TypeError('Unions of commands must serialize to lists of serialization data.');
  }
