...
function serializeComponentStyle (style: ComponentStyle) {
...
function deserializeComponentStyle (json: Record<string, any>): ComponentStyle {
...
export function createRestyleComponentCommand (initialStyle: ComponentStyle,
  newStyle: ComponentStyle,
  component: RestyleableComponent): SerializableCommand {
...
export function isRestylableComponent (component: AbstractComponent): component is RestyleableComponent {
  const hasMethods =
  ...
  static {
    SerializableCommand.register(defaultRestyleComponentCommandId, (json: any, _editor: Editor) => {
      ...
      if (typeof json.id !== 'string') {
        throw new TypeError(`json.id is of type ${typeof json.id}, not string.`);
      }
