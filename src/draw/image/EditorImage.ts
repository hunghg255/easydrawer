...
import type RenderingCache from '../rendering/caching/RenderingCache';
...
export function sortLeavesByZIndex (leaves: Array<ImageNode>) {
...
export default class EditorImage {
  ...
  public getElementsIntersectingRegion(region: Rect2, includeBackground = false) {
  ...
  private static AddComponentCommand = class extends SerializableCommand {
    ...
    public constructor(
      ...
      private applyByFlattening = false,
    ) {
      super('add-element');
      ...
      if (isNaN(element.getBBox().area)) {
        throw new TypeError('Elements in the image cannot have NaN bounding boxes');
      }
    }
  ...
  private settingExportRect = false;
 ...
 * @internal
 */
export function computeFirstIndexToRender (sortedLeaves: Array<ImageNode>, visibleRect?: Rect2) {
  let startIndex = 0;
...
export class ImageNode {
  ...
  private updateParents(recursive = false) {
    for (const child of this.children) {
      child.parent = this;
    ...
    if (this.children.length > this.targetChildCount * 10) {
      ...
      for (const child of this.children) {
        for (const [i, element] of grid.entries()) {
          if (element.containsRect(child.getBBox())) {
            indexToCount[i]++;
          }
        }
      }
  ...
  private checkRep(depth = 0) {
    ...
    if (debugMode) {
      ...
      const seenChildren = new Set<ImageNode>();
      for (const child of this.children) {
