...
export function pathFromRenderable (renderable: RenderablePathSpec): Path {
  if (renderable.path) {
 ...
 */
export function pathToRenderable (path: Path, style: RenderingStyle): RenderablePathSpecWithPath {
  return {
 ...
 */
function pathIncluded (renderablePath: RenderablePathSpec,
  path: Path): RenderablePathSpecWithPath {
 ...
 * @internal
 */
export function simplifyPathToFullScreenOrEmpty (renderablePath: RenderablePathSpec,
  visibleRect: Rect2,
 ...
 */
export function visualEquivalent (renderablePath: RenderablePathSpec,
  visibleRect: Rect2): RenderablePathSpecWithPath {
  const path = pathFromRenderable(renderablePath);
  const strokeWidth = renderablePath.style.stroke?.width ?? 0;
  const onlyStroked = strokeWidth > 0 && renderablePath.style.fill.a === 0;
  const styledPathBBox = path.bbox.grownBy(strokeWidth);
