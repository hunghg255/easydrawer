...
import { Rect2, Color4 } from '~/math';

import type CacheRecord from './CacheRecord';
import { type CacheState } from './types';
import { type ImageNode, computeFirstIndexToRender, sortLeavesByZIndex } from '../../image/EditorImage';
      ...
      if (!this.renderingIsUpToDate(leafIds)) {
        ...
        if (leafApproxRenderTime > this.cacheState.props.minProportionalRenderTimePerCache) {
          ...
          if (!this.cachedRenderer) {
            ...
            for (const [i, leaf] of leavesByIds.entries()) {
