...
import {
  ...
  type SVGLoaderUnknownAttribute,
  type SVGLoaderUnknownStyleAttribute,
  svgStyleAttributesDataKey,
} from '../../SVGLoader/SVGLoader';
import listPrefixMatch from '../../util/listPrefixMatch';
import type Viewport from '../../Viewport';
import type RenderablePathSpec from '../RenderablePathSpec';
import { pathFromRenderable } from '../RenderablePathSpec';
...
export default class SVGRenderer extends AbstractRenderer {
  ...
  private transformFrom(elemTransform: Mat33, elem: SVGElement, inCanvasSpace = false) {
  ...
  public override endObject(loaderData?: LoadSaveDataTable, elemClassNames?: string[]) {
    ...
    if (loaderData && !this.sanitize) {
      ...
      if (containerIDData && containerIDData[0]) {
        // If a string list,
        if ((containerIDData[0] as any).length > 0) {
