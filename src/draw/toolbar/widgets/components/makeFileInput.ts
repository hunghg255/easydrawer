 ...
 */
function makeFileInput (labelText: string,
    ...
    if (loading) {
      ...
      for (const [i, segment] of segments.entries()) {
        ...
        if (i % 2 === 1) {
          ...
          boldedText.textContent = segment;
          descriptionText.appendChild(boldedText);
        } else {
          descriptionText.appendChild(document.createTextNode(segment));
        }
      }
    }
  };
  ...
  const addFileEventListeners = () => {
    ...
    label.addEventListener('drop', (event: any) => {
      event.preventDefault();
      label.classList.remove('drag-target');
  ...
  if (customPickerAction) {
    ...
    input.addEventListener('click', promptForFiles);
  }
