 ...
 */
function makeGridSelector <ChoiceIdType>(labelText: string,
  defaultId: ChoiceIdType,
  choices: GridSelectChoice<ChoiceIdType>[]): GridSelector<ChoiceIdType> {
  ...
  let radiogroupName = `${toolbarCSSPrefix}-grid-selector-${idCounter++}`;
	...
	selectedValue.onUpdateAndNow((choiceId) => {
	  for (const button of buttons) {
	    button.setChecked(button.choiceRecord.id === choiceId);
	  }
	});
