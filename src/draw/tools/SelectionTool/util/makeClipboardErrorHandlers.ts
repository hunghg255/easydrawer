...
function makeClipboardErrorHandlers (editor: Editor) {
  ...
  return {
    onCopyError(error: unknown) {
      ...
      copyTextbox.addEventListener('copy', handleCopy);
      copyTextbox.addEventListener('dragstart', handleCopy);
    ...
    onPasteError(error: unknown) {
      ...
      pasteTextbox.addEventListener('paste', handlePaste);
      pasteTextbox.addEventListener('drop', handlePaste);
