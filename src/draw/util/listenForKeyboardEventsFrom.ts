 ...
 */
function listenForKeyboardEventsFrom (elem: HTMLElement, callbacks: Callbacks) {
	...
	const fireArtificalEventsBasedOn = (htmlEvent: EventWithKeyModifiers) => {
	  ...
	  for (const otherEvent of keysDown) {
	    ...
	    wasShiftDown ||= !!/^Shift(Left|Right)$/.test(code);
	    wasCtrlDown ||= !!/^Control(Left|Right)$/.test(code);
	    wasAltDown ||= !!/^Alt(Left|Right)$/.test(code);
	    wasMetaDown ||= !!/^Meta(Left|Right)$/.test(code);
	  }
