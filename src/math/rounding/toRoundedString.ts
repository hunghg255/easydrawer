 ...
 * ```
 */
export function toRoundedString (num: number): string {
  ...
  if (roundingDownMatch) {
    ...
    const lastDigit = Number.parseInt(postDecimalString.charAt(postDecimalString.length - 1), 10);
    const postDecimal = Number.parseInt(postDecimalString, 10);
    const preDecimal = Number.parseInt(roundingDownMatch[2], 10);
