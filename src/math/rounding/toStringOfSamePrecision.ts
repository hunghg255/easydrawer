...
export function toStringOfSamePrecision (num: number, ...references: string[]): string {
  ...
  if (nextDigit !== '') {
    const asNumber = Number.parseInt(nextDigit, 10);
    if (asNumber >= 5) {
      ...
      if (postDecimal.length > 0) {
        ...
        postDecimal = (Number.parseInt(postDecimal) + 1).toString();
      ...
      if (postDecimal.length === 0 || postDecimal.length > decimalPlaces) {
        preDecimal = (Number.parseInt(preDecimal) + 1).toString();
