import type BezierJSWrapper from './BezierJSWrapper';
import CubicBezier from './CubicBezier';
import LineSegment2 from './LineSegment2';
import type Parameterized2DShape from './Parameterized2DShape';
...
export function compareCurveIndices (a: CurveIndexRecord, b: CurveIndexRecord) {
 ...
 */
export function stepCurveIndexBy (index: CurveIndexRecord, stepBy: number): CurveIndexRecord {
...
export class Path {
  ...
  public closedContainsRect(rect: Rect2) {
    ...
    for (const edge of rect.getEdges()) {
      if (this.intersection(edge).length > 0) {
        return false;
      }
    }
  ...
  /**
	 ...
	 */
  public roughlyIntersects(rect: Rect2, strokeWidth = 0) {
    if (this.parts.length === 0) {
  ...
  /**
	 ...
	 * See also {@link fromString}.
	 */
  public toString(useNonAbsCommands?: boolean, ignoreCache = false): string {
  ...
  public static fromString(pathString: string): Path {
    ...
    const commandExp = /([achlmqstvz])\s*([^achlmqstvz]*)/gi;
    let current;
    while ((current = commandExp.exec(pathString)) !== null) {
      ...
      let numericArgs = argParts.map((arg) => Number.parseFloat(arg));
